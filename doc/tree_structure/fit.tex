...
and returns an element from the label set when beeing
called (Algorithm~\ref{alg:pred}, line 9). The loss
returned by $\gamma$ gets compared to the quality threshold
$\tau_l$. Is the loss $\leq \tau_l$ the classifier is good
...
be predictable, even though no classifier could be
...
returns a classifier with a loss $> \tau_l$. Since $|X|$ is
...
number generator\footnote{The implementation uses Python
3.6's random library.\cite[chapter 9.6]{python}} generating
a random value so that $lower(\beta_X[\text{dimension}])
\leq \text{random number} \leq upper(\beta_X[
\text{dimension}])$ (Algorithm~\ref{alg:fit}, line 8).
