...
\begin{figure*}
  ...
  \begin{subfigure}[b]{\textwidth}
    \centering
    \input{tree_structure/example_tree}
  ...
  \caption{Example of FIT on a dataset seen in Figure
    \ref{fig:scatter_example}. $\gamma$ simply computes the
      probability of all the elements in the partition,
      returns a function returning the label with the
      maximum probability and as loss 1 - the maximum
      probability:
      $\gamma(X,y) = f(z) \leftarrow l \in
      \text{label space where: }
      l = max(P(label(x \in X))), 1 - P(l)$. The
      thresholds are: $\tau_l = 1$, $\tau_{|X|} = 2$.
      $\tau_h$ can be any integer above 2.
    }
\end{figure*}
